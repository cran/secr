---
title: '**secr** 5.2 - spatially explicit capture--recapture in R'
...
The R package **secr** was written to address these weaknesses and allow for further development. It implements almost all the methods described by Borchers and Efford (2008), Efford et al. (2009), Efford (2011), Efford and Fewster (2013), Efford et al. (2013) and Efford and Mowat (2014). **secr** 5.2 uses external C++ code via package **Rcpp** for computationally intensive operations (Eddelbuettel and Francois 2011); Multi-threading on multiple CPUs with **RcppParallel** (Allaire et al. 2021) gives major speed gains. The most important functions of **secr** are listed in [Appendix 1](#appendix1).
...
**Table 4.** Vignettes for **secr** 5.2.
...
**secr** uses that term because it will be familiar to biologists. Three real parameters are commonly modelled in **secr** 5.2; these are denoted 'D' (for density), 'g0' (or 'lambda0') and 'sigma'. The last two real parameters, jointly define the model for detection probability as a function of location.

Models may be fitted by maximizing the full likelihood or the likelihood conditional on $n$, the number of individuals detected. Simple use of the conditional likelihoods (`CL = TRUE` in `secr.fit`) gives estimates of the detection parameters alone. D is then a derived parameter that is computed from an **secr** object with the function `derived` or one of its siblings (`derivedCluster` etc.), assuming uniform density. From **secr** 5.2 onwards, combining `CL = TRUE` with a formula for density (e.g., `model = D~x`) fits a model for *relative* density. The absolute density surface may be derived with function `derivedDsurface`.
...
These are the core functions of **secr** 5.2 -- the ones that you are most likely to use. S3 methods are marked with an asterisk.
