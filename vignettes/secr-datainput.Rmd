...
recognised automatically as comma-delimited. This makes it feasible to
prepare input files with spreadsheet software. Care should be taken to
prefix header information with the comment character (default #).
...
file (below)[^footnote1]

[^footnote1]: The older, but still supported, 'XY' format uses
...
and y-coordinates for each detector, one per line:
...
Our stoat example is very simple: apart from specifying the input file names we only need to alter the detector type (see `?detector`). The number of occasions (7) will be determined automatically from the input and there are no individual covariates to be named. The data are in the folder 'extdata' of the
package installation, so we set the working directory to there.
...
allow repeat detections are called 'count' detectors in **secr**. [In
fact, proximity detectors are a special case of count detectors in
which the count always has a Bernoulli distribution]. Count data can
...
Signal strength detectors are described in the document [secr-sound.pdf] (see also Efford et al. 2009). Here we just note that signal strength data may be input with `read.capthist` using a minor
...
####Polygon, transect and telemetry detectors
...
The 'telemetry' detector type is used to input animal locations from radiotelemetry. For input it works much the same as a polygon detector; while a perimeter is required, it is not used in analysis and therefore may be as crude as you like.
...
An alternative function for text file input is
`read.SPACECAP`. This uses the formats described in the help
for the R package SPACECAP (Singh et al. 2010).
...
Efford, M. G. (2012) *DENSITY 5.0: software for spatially
  ...
  Zealand http://www.otago.ac.nz/density.
...
Singh, P., Gopalaswamy, A. M., Royle, A. J., Kumar, N. S. and
  Karanth, K. U. (2010) *SPACECAP: A program to estimate animal
    abundance and density using Bayesian spatially explicit
    capture-recapture models. Version 1.0.* Wildlife Conservation
  Society - India Program, Centre for Wildlife Studies, Bangalure,
  India.
...
In conventional capture--recapture, 'occasion' refers to a discrete sampling event (e.g., Otis et al. (1978) and program CAPTURE). A typical 'occasion' is a daily trap visit, but the time
  interval represented by an 'occasion' varies widely between studies. Although trapped samples accumulate over an interval (e.g., the preceding day), for analysis they are treated as instantaneous. Occasions are numbered 1, 2, 3, etc. Closed population analyses usually require two or more occasions (see
  Miller et al. 2005 for an exception).
