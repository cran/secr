---
title: '**secr** 3.1 - spatially explicit capture--recapture in R'
...
```{r, echo = FALSE, eval=TRUE, fig.height=3.5, fig.width=7, cache = TRUE}
...
documented. Telemetry data are used to augment capture--recapture data 
(see `addTelemetry` and [secr-telemetry.pdf]).
...
external C code for computationally intensive operations. [Appendix 1](#appendix1) compares the features of DENSITY and **secr**. The most important functions of **secr** are listed in [Appendix 2](#appendix2).
...
To perform an SECR analysis you explicitly or implicitly construct each of these objects in turn. Fig. 2 summarizes the relationships among the classes.  
...
- Fitted secr models may be further manipulated with the methods shown on the right and others listed in [Appendix 3](#appendix3). 
...
Other documentation, in the form of pdf vignettes built with **knitr**,  will be added from time to time. The 'User guides...' link in the package help index lists available files. The vignettes in Table 4 are included in **secr** 3.1 or may be found on the Density website.

\vspace{12pt}

**Table 4.** Vignettes for **secr** 3.1.
...
Three real parameters are commonly modelled in **secr** 3.1; these are denoted 'D' (for density), 'g0' (or 'lambda0') and 'sigma'. Only the last two real parameters, which jointly define the model for detection probability as a function of location, can be estimated directly when the model is fitted by maximizing the conditional likelihood (`CL = TRUE` in `secr.fit`).  D is then a derived parameter that is computed from an **secr** object with the function `derived` or one of its siblings (`derived.cluster` etc.). 
...
The standard models for 'real' parameters in **secr** are linear on the link scale, much like a generalised linear model. For more flexibility is possible to use semi-parametric 'regression spline' smooths. These are implemented in **secr** using a method suggested by Borchers and Kidney (2014): Simon Wood's R package **mgcv** is used to parse s() and te() terms in model formulae and construct basis functions that are used like linear covariates within **secr**. Any 'real' parameter may be modelled with regression splines (D, lambda0, sigma, noneuc etc.). For details see the help page (?smooths) and the documentation for **mgcv**.
...
These are the core functions of **secr** 3.1 -- the ones that you are most likely to use. S3 methods are marked with an asterisk.
...
| `split`*     	| split a single-session capthist by various criteria |
...
#Appendix 5. More on models in **secr** {#appendix5}
...
[secr-habitatmasks.pdf]: http://www.otago.ac.nz/density/pdfs/secr-habitatmasks.pdf
[secr-markresight.pdf]: http://www.otago.ac.nz/density/pdfs/secr-markresight.pdf
[secr-multisession.pdf]: http://www.otago.ac.nz/density/pdfs/secr-multisession.pdf
...
[secr-telemetry.pdf]: http://www.otago.ac.nz/density/pdfs/secr-telemetry.pdf
[secr-tutorial.pdf]: http://www.otago.ac.nz/density/pdfs/secr-tutorial.pdf
[secr-troubleshooting.pdf]: http://www.otago.ac.nz/density/pdfs/secr-troubleshooting.pdf
