---
title: '**secr** 3.0 - spatially explicit capture--recapture in R'
...
Other documentation, in the form of pdf vignettes built with **knitr**,  will be added from time to time. The 'User guides...' link in the package help index lists available files. The vignettes in Table 4 are included in **secr** 3.0 or may be found on the Density website.

\vspace{12pt}

**Table 4.** Vignettes for **secr** 3.0.
...
Three real parameters are commonly modelled in **secr** 3.0; these are denoted 'D' (for density), 'g0' (or 'lambda0') and 'sigma'. Only the last two real parameters, which jointly define the model for detection probability as a function of location, can be estimated directly when the model is fitted by maximizing the conditional likelihood (`CL = TRUE` in `secr.fit`).  D is then a derived parameter that is computed from an **secr** object with the function `derived` or one of its siblings (`derived.cluster` etc.). 
...
These are the core functions of **secr** 3.0 -- the ones that you are most likely to use. S3 methods are marked with an asterisk.
