...

{
    ...
    runone <- function(i=1) {
        ...
        unobspop <- sim.popn(0, data.frame(x=0,y=0), covariates = NULL) 

        if (N>nrow(CH)) {
            detpar <- detectpar(object, byclass = TRUE)  
            ...
            for (i in 1:length(detpar)) {
                pd <- pdot(X = mask, traps = traps(CH), detectfn = object$detectfn,
                           detectpar = detpar[[i]], noccasions = ncol(CH), ncores = ncores)
