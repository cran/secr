...
## groups to be found in covariates(object$capthist) cf 
    ...
    if (is.list(object$capthist) & is.null(sessnum)) {
        ...
        getderived <- function (selection) {
                ...
                derivedmean <- c(mean(selected.a), sum(1/selected.a) )
                derivedSE <- c(NA, NA)
                varcomp1 <- c(NA, NA)
                varcomp2 <- c(NA, NA)
                ...
                if (loginterval) {
                    temp$lcl <- derivedmean / exp(z * sqrt(log(1 + derivedSE^2 / derivedmean^2)))
                    temp$ucl <- derivedmean * exp(z * sqrt(log(1 + derivedSE^2 / derivedmean^2)))
                }
                else {
